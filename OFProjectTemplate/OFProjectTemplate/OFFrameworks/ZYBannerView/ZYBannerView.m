...
- (void)fixDefaultPosition
{
    ...
    if (self.shouldLoop) {
        ...
        // 第0个item
        [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:self.pageNum inSection:0]
                                    atScrollPosition:UICollectionViewScrollPositionLeft animated:NO];
        self.pageControl.currentPage = 0;
    }
}
...
{   [self.delegate scrollviewPageWithOffset:scrollView.contentOffset.x];
    // 用户停止滑动的时候开启定时器
    [self startTimer];
}
...
{
     [self.delegate scrollviewWithOffset:scrollView.contentOffset.x];
